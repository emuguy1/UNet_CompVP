...
class DepthDataset(Dataset):
    def __init__(self, images_dir: str, depth_dir: str, scale: float = 1.0):
        self.images_dir = Path(images_dir)
    ...
    @staticmethod
    def preprocess(pil_img, scale, is_depth):
        w, h = pil_img.size
        newW, newH = int(scale * w), int(scale * h)
        assert newW > 0 and newH > 0, 'Scale is too small, resized images would have no pixel'
        pil_img = pil_img.resize((newW, newH), resample=Image.NEAREST if is_depth else Image.BICUBIC)
        img = np.asarray(pil_img)
    ...
    def __getitem__(self, idx):
        ...
        assert len(img_file) == 1, f'Either no image or multiple images found for the ID {name}: {img_file}'
        assert len(depth_file) == 1, f'Either no depth map or multiple depth maps found for the ID {name}: {depth_file}'
